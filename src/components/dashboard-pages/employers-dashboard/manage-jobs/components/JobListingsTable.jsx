...
export default function JobListingsTable() {
  ...
  const { industries, categories } = useSelector((state) => state.data);
  useEffect(() => {
  ...
  const handleDeleteJob = async () => {
    try {
      const response = await fetch(
        `https://api.sentryspot.co.uk/api/employeer/job-post/${jobToDelete}`,
        {
          method: "DELETE",
          headers: {
            Authorization: token,
  ...
  const handlePublishJob = async (id, currentStatus) => {
    ...
    try {
      const response = await fetch(
        `https://api.sentryspot.co.uk/api/employeer/job-status/${id}`,
        {
          method: "PUT",
          headers: {
            Authorization: token,
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to ${currentStatus === 0 ? "publish" : "unpublish"} job`
        );
      }

      toast.success(
        `Job ${currentStatus === 0 ? "published" : "unpublished"} successfully`
      );
      ...
      toast.error(
        error.message ||
          `Error ${currentStatus === 0 ? "publishing" : "unpublishing"} job`
      );
    }
  };

  const shareJob = (jobId) => {
    const jobUrl = `https://sentryspot.co.uk/${jobId}`; // Update with your actual job link
        ...
        <main className="flex-1 pe-2">
          ...
          <div className="w-full overflow-x-auto border">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-blue-100">
                <tr>
                  <th className="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase whitespace-nowrap">
                    CREATED ON
                  </th>
                    ...
                    <tr key={job.job_detail.id}>
                      ...
                      <td className="px-6 py-4 text-md font-medium text-green-600">
                        ...
                        <div className="text-md text-gray-600 flex flex-wrap gap-2">
                          <span className="px-2 py-1 bg-gray-100 rounded-md">
                            {job.job_detail.experience_id_min || "0"} yrs
                          </span>
                          <span className="px-2 py-1 bg-gray-100 rounded-md">
                            {job.job_detail?.location}
                          </span>
                          <span className="px-2 py-1 bg-gray-100 rounded-md">
                            {getIndustryName(job.job_detail.industry_id)}
                          </span>
                          <span className="px-2 py-1 bg-gray-100 rounded-md">
                            {getCategoryName(job.job_detail.category_id)}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-md text-gray-500">
                        <p className="bg-pink-200 px-2 p-1 rounded-3xl text-center whitespace-nowrap">
                          {job.job_detail.job_status_id === 1
                            ? `Published 🖋️`
                            : `Unpublished 🖊`}
                        </p>
                      </td>
                      <td className="px-6 py-4 text-md text-gray-700 whitespace-nowrap">
                        ...
                        <Link to={"/employers-list"}>
                          <button className=" text-md underline text-blue-700">
                            2143 Applied
                          </button>
                        </Link>
                      </td>
                        ...
                        <div className="flex space-x-2">
                          ...
                          {job.job_detail.job_status_id === 0 ? (
                            <FaCheckCircle
                              className="w-4 h-4 text-blue-700 cursor-pointer"
                              onClick={() =>
                                handlePublishJob(job.job_detail.id, 0)
                              }
                            />
                          ) : (
                            <FaBan
                              ...
                              onClick={() =>
                                handlePublishJob(job.job_detail.id, 1)
                              }
                            />
                          )}
                          <FaLinkedinIn
          ...
          {showPopup && (
            <PopupForm
              ...
              onClose={closePopup}
              title={
                data.find((job) => job.job_detail.id === selectedJobId)
                  ?.job_detail.job_title
              }
              cities={
                data.find((job) => job.job_detail.id === selectedJobId)?.cities
                  ?.name
              }
              states={
                data.find((job) => job.job_detail.id === selectedJobId)?.states
                  ?.name
              }
              countries={
                data.find((job) => job.job_detail.id === selectedJobId)
    ...
    <div className="absolute top-0   items-center justify-center col-lg-12 bg-blue-800 overflow-y-auto h-4/5 p-2 rounded-xl">
      ...
      >
        ...
        >
          <path
            fillRule="evenodd"
            d="M10 9.293l4.707-4.707a1 1 0 00-1.414-1.414L10 7.586 5.707 3.293A1 1 0 104.293 4.707L8.586 9l-4.293 4.293a1 1 0 001.414 1.414L10 10.414l4.293 4.293a1 1 0 001.414-1.414L10 9.293z"
            clipRule="evenodd"
          />
        </svg>
      </button>
        ...
        <form className="default-form" onSubmit={handleSubmit}>
          ...
          <div className="form-group col-lg-12 col-md-12 mt-4">
            ...
            <ReactQuill
              ...
              className={`mt-1 border border-gray-300 h- rounded-md shadow-sm ${
          ...
          <div className="form-group col-lg-12 col-md-12 text-right">
            <button className="theme-btn btn-style-one" type="submit"></button>
          </div>
