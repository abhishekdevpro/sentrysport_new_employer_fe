...
const ChatBoxContentField = () => {
  ...
  const loadChatMessages = async (chatUserId, lastId) => {
    try {
      ...
      const response = await axios.get(
        `https://api.sentryspot.co.uk/api/employeer/chat-user-list-messages?chat_user_id=${chatUserId}&last_message_id=${lastId}`,
        {
          headers: {
            'Authorization': `${token}`,
          }
  ...
  const fetchChatHistory = async (chatUserId) => {
    try {
      ...
      const token = localStorage.getItem(Constant.USER_TOKEN);
      const response = await axios.get(
        `https://api.sentryspot.co.uk/api/employeer/chat-user-list-messages?chat_user_id=${chatUserId}&last_message_id=0`,
        {
          headers: {
            'Authorization': `${token}`,
          }
