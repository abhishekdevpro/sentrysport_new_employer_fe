...
const LocationSelector = ({ register, setValue, defaultLocation }) => {
  ...
  // Keep query in sync with defaultLocation
  useEffect(() => {
    if (defaultLocation && defaultLocation !== query) {
      setQuery(defaultLocation);
    }
  ...
  const fetchLocations = useCallback(
    debounce(async (searchTerm) => {
      ...
      try {
        const response = await axios.get(
          `https://api.sentryspot.co.uk/api/jobseeker/locations?locations=${encodeURIComponent(searchTerm)}`
        );

        const locationList = response.data?.data?.location_names || [];
        setLocations(locationList);
        setShowDropdown(locationList.length > 0);
      } catch (error) {
        console.error("Error fetching locations:", error);
        setLocations([]);
        setShowDropdown(false);
  ...
  const handleInputChange = (e) => {
    ...
    fetchLocations(value); // debounced API call
  };
